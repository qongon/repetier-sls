...
#endif
bool HALRTC::isTimeSet = false;
#ifdef HAS_XTAL_RTC  
int HALRTC::RTCOffset; 
RTCDue HALRTC::RTCSource(0); 
#endif 
...
void HALRTC::setupRTC(bool type) {
#ifdef HAS_XTAL_RTC  
    if (type) {
        // We NEED to clear out the RTC_CR register just in case we crashed the RTC.
        // (Attempting to set RTC_CR_UPDTIM twice within the same second.)
        // Power cycling the device works too. Or a delay between sets.  
        // If we don't do anything, RTC_SR_ACKUPD will never be set again, leading to a 
        // endless loop in the default SAM3X8A RTC library. 
...
char* HALRTC::getRTCTimeStr() { 
    ...
    RTCSource.getTime(&hour, &minutes, &seconds);
    if (RTCOffset) { 
        int hourOffset = 0, minuteOffset = 0;
        hourOffset = (RTCOffset-(minuteOffset = (RTCOffset % 60))) / 60;
        hour += hourOffset; minutes += minuteOffset; 
        minutes = minutes < 0 ? minutes + 60 : minutes > 60 ? minutes - 60 : minutes; 
        hour = hour < 1 ? hour + 24 : hour > 24 ? hour - 24 : hour;  
    } 
    bool ampm = hour < 12;
    hour = hour > 12 ? hour - 12 : hour;  
    //0:00:00 PM 
    snprintf(buf, 15, "%02d:%02d:%02d %2s",(uint8_t)hour, (uint8_t)minutes, (uint8_t)seconds, (ampm ? "AM" : "PM"));
    return buf;
#endif
    return nullptr;
}
char* HALRTC::getRTCDateStr() { 
#ifdef HAS_XTAL_RTC
    if (!isTimeSet) {
        return nullptr;
    } 
    ...
    int day = 0, month = 0; 
    uint16_t year = 0;
    RTCSource.getDate(0, &day, &month, &year);
    if (RTCOffset) { 
        int hour = RTCSource.getHours(); 
        int hourOffset = 0;
        hourOffset = (RTCOffset-((RTCOffset % 60))) / 60;
        hour += hourOffset;  //TODO: Account for minutes as well.
        if(hour < 1) {
            day--; 
        }
        else if(hour > 24) {
    ...
    //0000-00-00 
    snprintf(buf, 14, "%d-%02d-%02d",(uint16_t)year, (uint8_t)month, (uint8_t)day);
    return buf;
#endif
    return nullptr;
}
void HALRTC::setRTCTime(uint32_t unixtime, int offset) {
#ifdef HAS_XTAL_RTC       
    RTCOffset = offset == 1 ? 0 : unixtime == 0 ? 0 : offset; 
    if (unixtime != 1) { 
        isTimeSet = unixtime == 0 ? false : true;
        RTCSource.setClock(unixtime);  
    } 
    Com::printFLN(isTimeSet ? PSTR("Clock set! ") : PSTR("Clock cleared!")); 
    if (!isTimeSet) {
        return;
    }
    Com::printF(getRTCTimeStr());
    Com::printFLN(" | ", getRTCDateStr());
    return;
#endif
    Com::printErrorFLN(PSTR("RTC Support not compiled into this version!"));
}
void HALRTC::setRTCTime(char* textTime) {
#ifdef HAS_XTAL_RTC  
    char c; 
    char time[9]{0}, date[11]{0}; 
    int t = 0, d = 0, mode = 0, hasP = 0, hasS = 0;
    while((c = *(textTime++)) != '\0') { 
        if (c == 'S' || c == 'P') {
            if ((c == 'S' && hasS) || (c == 'P' && hasP)) {
                return;
            } 
            mode = (c == 'S') ? (hasS = 1) : (hasP = -1);
            continue;
        }  
        if (mode > 0 && t < 8) {
            time[t++] = c; 
    ...
    //2000-0-0 etc
    if (d > 7) {  
        int chunk = 0, year = 0, month = 0, day = 0, i = sizeof(date), mult = 1; 
        while(i-- > 0) { 
            c = date[i];
            if (c == ' ' || c =='\0') {
                continue;
        ...
        if (year < 2000 || month > 12 || day > 32 || day < 0 || month < 0 || year > 3000) {
            return;
        }
        RTCSource.setDate(day, month, year); 
        isTimeSet = true;
        return;
    } 
    if (t > 4) {
        RTCOffset = 0;
        RTCSource.setTime(time); 
        isTimeSet = true;
        return;
    } 
#endif
